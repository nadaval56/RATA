...
/* ================================================================
   ...
   הפעלה:
     ...
     node audit.mjs --root .                       # סורק את כל דפי ה-HTML
     node audit.mjs --root . --pages / /about/     # דפים מסוימים
     node audit.mjs --base http://localhost:3000 --pages / /about/
...
const PAGES = list('pages') || (BASE_ARG ? ['/'] : discover(ROOT));
  ...
  const measure = () => p.evaluate(() => {
    ...
    document.querySelectorAll('h1,h2,h3,p,li,td,a,button,span').forEach(e => {
      ...
      /* סרגל בחירת הגודל עצמו אינו אמור לגדול — הוא העוגן שמולו
         ...
         כולם גדלים" על כל דף, כולל דף קפוא לחלוטין. בדיוק הטעות
         שהקובץ הזה נכתב כדי לתפוס. */
